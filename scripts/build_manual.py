    ...
    (
        ...
        [
            ...
            p(
                ...
                "its SQLite path from AVENUE_GUARD_DB_PATH first, then database.path in config.json, and only then the local default. "
                "For production, point that value at a mounted Persistent Disk path such as /var/data/avenue-guard/bot.db."
            ),
            p(
                "The /bot storage command checks the running path and the latest backup record. The /bot backup command creates a zipped "
                "copy immediately, and the background backup loop posts scheduled copies to the configured backup channel. This gives the "
                "bot a primary durable database plus a Discord-visible recovery trail."
            ),
            table(
                ["Table", "Purpose"],
                [
