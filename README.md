...
Uptime setup establishes its startup boundary before background writers start. Guarded checkpoints retry after short queue deferrals, retain online/offline transitions while the process survives, and cannot count an already committed interval twice. Read-only public status snapshots do not trigger initialization writes; the admin dashboard shows pending uptime persistence and flags sustained delays. Release bootstrap retries independently of the status loop. Expired GD cache cleanup no longer runs before request tasks start: read-first maintenance deletes at most 200 expired rows per transaction and backs off when the writer is busy. Busy diagnostics include the transaction label and owning task.
